 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class tl{constructor(t){this.apiKey=t}getGenerativeModel(t,e){if(!t.model)throw new x("Must provide a model name. Example: genai.getGenerativeModel({ model: 'my-model-name' })");return new tc(this.apiKey,t,e)}getGenerativeModelFromCachedContent(t,e,n){if(!t.name)throw new B("Cached content must contain a `name` field.");if(!t.model)throw new B("Cached content must contain a `model` field.");for(let n of["model","systemInstruction"])if((null==e?void 0:e[n])&&t[n]&&(null==e?void 0:e[n])!==t[n]){if("model"===n&&(e.model.startsWith("models/")?e.model.replace("models/",""):e.model)===(t.model.startsWith("models/")?t.model.replace("models/",""):t.model))continue;throw new B(`Different value for "${n}" specified in modelParams (${e[n]}) and cachedContent (${t[n]})`)}let s=Object.assign(Object.assign({},e),{model:t.model,tools:t.tools,toolConfig:t.toolConfig,systemInstruction:t.systemInstruction,cachedContent:t});return new tc(this.apiKey,s,n)}}(0,S.auth).onAuthStateChanged(async t=>{t?await tv():window.location.href="not-logged-in.html"});const td=document.getElementById("logoutBtn");document.getElementById("biometricAuthBtn");const tu=document.getElementById("addTransactionBtn"),th=document.getElementById("transactionModal"),tf=document.getElementById("transactionForm"),tg=document.getElementById("cancelBtn"),tm=document.getElementById("transactionsList"),tE=document.getElementById("chatbotModal"),tp=document.getElementById("closeChatbot"),tC=document.getElementById("chatInput"),ty=document.getElementById("sendChat"),tO=document.getElementById("chatbotMessages");async function tv(){let t=S.auth.currentUser;if(t)try{let e=await (0,b.getDocs)((0,b.query)((0,b.collection)(S.db,"transactions"),(0,b.where)("userId","==",t.uid))),n=0,s=0;tm.innerHTML=e.empty?"No transactions yet":"",e.forEach(t=>{let e=t.data();"income"===e.type?n+=parseFloat(e.amount):s+=parseFloat(e.amount);let o=document.createElement("div");o.className=`transaction ${e.type}`,o.innerHTML=`${e.description} ${"income"===e.type?"+":"-"}$${parseFloat(e.amount).toFixed(2)}`;let i=document.createElement("button");i.textContent="Delete",i.className="delete-btn",i.addEventListener("click",async()=>{try{await (0,b.deleteDoc)((0,b.doc)(S.db,"transactions",t.id)),tT("Transaction deleted successfully!"),tv()}catch(t){tT("Error deleting transaction: "+t.message,"error")}}),o.appendChild(i),tm.appendChild(o)}),document.getElementById("totalBalance").textContent=`$${(n-s).toFixed(2)}`,document.getElementById("totalIncome").textContent=`$${n.toFixed(2)}`,document.getElementById("totalExpenses").textContent=`$${s.toFixed(2)}`}catch(t){tT("Error loading transactions: "+t.message,"error")}}async function tI(t){try{let e=new tl("AIzaSyCKLHTM-Bb9rOnWqLOzBV8GC8_xsHETjpA").getGenerativeModel({model:"gemini-1.5-flash"});return(await e.generateContent(t)).response.text()}catch(t){return"Error: Unable to process your request."}}function t_(t,e){let n=document.createElement("div");n.className="chat-message",n.innerHTML=`${t}: ${e}`,tO.appendChild(n),tO.scrollTop=tO.scrollHeight}function tT(t,e="success"){let n=document.createElement("div");n.className=`toast ${e}`,n.textContent=t,document.getElementById("toastContainer").appendChild(n),setTimeout(()=>n.remove(),3e3)}td.addEventListener("click",()=>{(0,w.signOut)(S.auth).then(()=>{console.log("Signed out successfully!"),window.location.href="index.html"}).catch(t=>{console.error("Error signing out:",t.message)})}),tu.addEventListener("click",()=>{th.classList.add("active")}),tg.addEventListener("click",()=>{th.classList.remove("active"),tf.reset()}),tf.addEventListener("submit",async t=>{t.preventDefault();let e=t.target.description.value,n=parseFloat(t.target.amount.value),s=t.target.type.value,o=S.auth.currentUser;if(!(!o||!e||isNaN(n))&&s)try{await (0,b.addDoc)((0,b.collection)(S.db,"transactions"),{description:e,amount:n,type:s,userId:o.uid,createdAt:new Date}),tT("Transaction added successfully!"),tv(),th.classList.remove("active"),tf.reset()}catch(t){tT("Error adding transaction: "+t.message,"error")}}),document.getElementById("chatbotButton").addEventListener("click",()=>{tE.classList.toggle("hidden")}),tp.addEventListener("click",()=>{tE.classList.add("hidden")}),ty.addEventListener("click",async()=>{let t=tC.value.trim();if(t){t_("You",t),tC.value="";try{let e=await tI(t);t_("Gemini AI",e)}catch(t){tT("Error processing chat request: "+t.message,"error")}}});
//# sourceMappingURL=transactions.e1c9dd26.js.map
